...
# QS.ancp2pb
QS.ancp2pb<-function(x1,y1,x2,y2,pts=NULL,nboot=500,alpha=.05,method='BH',xout=FALSE,outfun=outpro,
SEED=TRUE,MC=FALSE){
#
#  ANCOVA based on a quantile shift measure of effect size and the running interval smoother
#  Neither group is viewed as the control group.  
# Only one covariate is allowed
#
# if npts=NULL, points are taken to be the quartiles of the first independent variable, x1
#  
#
if(SEED)set.seed(2)
xy<-cbind(x1,y1)
...
if(xout){
flag1=outfun(x1,plotit=FALSE)$keep
flag2=outfun(x2,plotit=FALSE)$keep
x1=x1[flag1]
x2=x2[flag2]
y1=y1[flag1]
y2=y2[flag2]
...
npts=length(pts)
output=matrix(NA,npts,6)
dimnames(output)=list(NULL,c('Pts','Est','ci.low','ci.up','p.value','adj,p.value'))
output[,1]=pts
output[,2]=as.vector(matl(e))
output[,3]=ci[,1]
output[,4]=ci[,2]
output[,5]=A
output[,6]=p.adjust(A,method=method)
output
list(n1=n1,n2=n2,output=output)
}
...
# QS.ancp2.sub
QS.ancp2.sub<-function(m,pts){
v=QSanc(m[[1]],m[[2]],m[[3]],m[[4]],pts=pts)
v=as.vector(matl(matl(v)))
v
}

ancova.smKMS<-
function(x1,y1,x2,y2,fr1=1,fr2=1,alpha=.05,sm=FALSE,est=tmean,tr=.2,method='BH',nboot=500,
plotit=TRUE,pts=NA,xout=FALSE,outfun=out,LP=TRUE,...){
#
# Compare two independent  groups using  
# with KMS measure of effect size in cojunction with  a covariate
# No parametric assumption is made about the form of
# the regression lines--a running interval smoother is used.
#
#  Assume data are in x1 y1 x2 and y2
#
#  The plot is based on measure of location indicated by the argument
#  est. Default is the a 20% trimmed mean
#
#   LP=TRUE: use running interval smoother followed by LOESS
#
if(ncol(as.matrix(x1))>1)stop('One covariate only is allowed with this function')
...
isub[4]<-floor((isub[3]+isub[5])/2)
mat<-matrix(NA,5,8)
dimnames(mat)<-list(NULL,c('X','n1','n2','Effect.Sizet','ci.low','ci.hi','p.value','adj.p.value'))
for (i in 1:5){
g1<-y1[near(x1,x1[isub[i]],fr1)]
g2<-y2[near(x2,x1[isub[i]],fr2)]
g1<-g1[!is.na(g1)]
g2<-g2[!is.na(g2)]
test<-KMS.ci(g1,g2,tr=tr,alpha=alpha)
mat[i,1]<-x1[isub[i]]
mat[i,2]<-length(g1)
mat[i,3]<-length(g2)
mat[i,4]<-test$effect.size
mat[i,5]<-test$ci[1]
mat[i,6]<-test$ci[2]
mat[i,7]<-test$p.value
}}
...
mat<-matrix(NA,length(pts),8)
dimnames(mat)<-list(NULL,c('X','n1','n2','Effect.Size','ci.low','ci.hi','p.value','adj.p.value'))
for (i in 1:length(pts)){
g1<-y1[near(x1,pts[i],fr1)]
g2<-y2[near(x2,pts[i],fr2)]
g1<-g1[!is.na(g1)]
g2<-g2[!is.na(g2)]
test=KMS.ci(g1,g2,tr=tr,alpha=alpha,nboot=nboot)
mat[i,1]<-pts[i]
mat[i,2]<-length(g1)
mat[i,3]<-length(g2)
if(length(g1)<=5)print(paste('Warning, there are',length(g1),' points corresponding to the design point X=',pts[i]))
...
mat[,8]=p.adjust(mat[,7],method=method)
if(plotit){
runmean2g(x1,y1,x2,y2,fr=fr1,est=est,sm=sm,xout=FALSE,LP=LP,tr=tr,...)
}
list(output=mat)
}

ancova.smQS<-
function(x1,y1,x2,y2,fr1=1,fr2=1,alpha=.05,sm=FALSE,locfun=median,nboot=500,method='BH',SEED=TRUE,
est=tmean,plotit=TRUE,pts=NA,xout=FALSE,outfun=out,LP=TRUE,...){
#
# Compare two independent  groups using  
# with KMS measure of effect size in cojunction with  a covariate
# No parametric assumption is made about the form of
...
#  Assume data are in x1 y1 x2 and y2
#
#. locfun: measure of location used by the quantile shift effect size
#
#  The plot is based on measure of location indicated by the argument
#  est. 
#   Default is the a 20% trimmed mean
#
#   LP=TRUE: use running interval smoother followed by LOESS
#
if(ncol(as.matrix(x1))>1)stop('One covariate only is allowed with this function')
...
g1<-g1[!is.na(g1)]
g2<-g2[!is.na(g2)]
test=QSci(g1,g2,locfun=locfun,alpha=alpha,nboot=nboot,SEED=SEED)
mat[i,1]<-pts[i]
mat[i,2]<-length(g1)
mat[i,3]<-length(g2)
if(length(g1)<=5)print(paste('Warning, there are',length(g1),' points corresponding to the design point X=',pts[i]))
...
# ======================================================================
# Function aliases (moved to end to resolve forward reference issues)
# ======================================================================
